      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="flex gap-3">
              {['Twitter', 'Instagram', 'Youtube'].map(s => (
                <div key={s} className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 cursor-pointer transition-colors">
                  <Icon name="Globe" size={14} className="text-background/70" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-background/60 mb-3">Принимаем по всему миру</p>
            <div className="flex flex-wrap gap-2">
              {['💳 Visa', '💳 MC', '₿ Crypto', '🏦 SWIFT', '💰 PayPal'].map(p => (
                <span key={p} className="px-2 py-1 bg-white/10 rounded-md text-xs text-background/70">
                  {p}
                </span>
              ))}
            </div>
        ...
        <div className="border-t border-white/10 mt-8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-background/40">
